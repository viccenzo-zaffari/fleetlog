...
const STYLES = `
  ...
  * { -webkit-tap-highlight-color: transparent; }
  .app { max-width: 100%; }
  @media (min-width: 480px) { .app { max-width: 480px; } }

  /* FULLSCREEN IMAGE VIEWER */
  .img-viewer { position: fixed; inset: 0; background: rgba(0,0,0,0.95); z-index: 500; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; }
  .img-viewer img { max-width: 100%; max-height: 80vh; object-fit: contain; border-radius: 12px; }
  .img-viewer-close { position: absolute; top: 16px; right: 16px; background: rgba(255,255,255,0.15); border: none; border-radius: 50%; width: 40px; height: 40px; color: white; font-size: 20px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .img-viewer-label { color: rgba(255,255,255,0.6); font-size: 13px; margin-top: 16px; }

  /* BIGGER TOUCH TARGETS */
  .nav-item { padding: 4px 12px; min-width: 60px; }
  .chip { padding: 8px 14px; font-size: 13px; }
  .timeline-item { padding: 16px 0; }
  .quick-btn { padding: 18px 14px; }
  .btn-primary, .btn-secondary { padding: 16px; font-size: 16px; }
  .form-input { padding: 14px; font-size: 16px; } /* 16px prevents iOS zoom */
  .receipt-upload { padding: 24px 16px; min-height: 100px; }
  .photo-slot { min-height: 100px; }

  /* CAROUSEL MOBILE */
  .carousel-wrap { touch-action: pan-y; }
  .carousel-slide { height: 240px; }
  @media (min-height: 700px) { .carousel-slide { height: 280px; } }

  /* RECEIPT CLICKABLE */
  .receipt-clickable { cursor: pointer; position: relative; }
  .receipt-clickable::after { content: '🔍'; position: absolute; bottom: 6px; right: 6px; background: rgba(0,0,0,0.6); border-radius: 50%; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; font-size: 14px; }
...
function FleetApp({ session }) {
  ...
  const [fullImg, setFullImg]         = useState(null) // { url, label }
    ...
    return (
      <div style={{ margin: "0 -20px" }}>
        ...
        <div className="carousel-wrap"
          onTouchStart={e => { const t = e.touches[0]; e._startX = t.clientX }}
          onTouchEnd={e => {
            const dx = e.changedTouches[0].clientX - (e._startX || e.changedTouches[0].clientX)
            if (dx < -50 && carouselIdx < slides.length - 1) setCarouselIdx(p => p + 1)
            if (dx >  50 && carouselIdx > 0) setCarouselIdx(p => p - 1)
          }}>
              ...
              return (
                <div className="timeline-item" key={rec.id} onClick={() => setViewRecord(rec)}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    ...
                    {rec.receipt_url && <img src={rec.receipt_url} alt="NF" className="receipt-thumb receipt-clickable" onClick={e => { e.stopPropagation(); setFullImg({ url: rec.receipt_url, label: 'Nota Fiscal — ' + rec.title }) }} />}
                    <button
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {viewRecord && (
          <div className="overlay" onClick={e => e.target === e.currentTarget && setViewRecord(null)}>
            <div className="sheet">
              ...
              {viewRecord.receipt_url && <div style={{ marginBottom: 14 }}><div className="form-label" style={{ marginBottom: 8 }}>🧾 Nota Fiscal <span style={{fontSize:11,color:'var(--accent)',fontWeight:400}}>— toque para ampliar</span></div><img src={viewRecord.receipt_url} alt="NF" className="record-detail-img receipt-clickable" style={{cursor:'pointer'}} onClick={() => setFullImg({ url: viewRecord.receipt_url, label: 'Nota Fiscal — ' + viewRecord.title })} /></div>}
        ...
        {/* FULLSCREEN IMAGE VIEWER */}
        {fullImg && (
          <div className="img-viewer" onClick={() => setFullImg(null)}>
            <button className="img-viewer-close" onClick={() => setFullImg(null)}>✕</button>
            <img src={fullImg.url} alt={fullImg.label} onClick={e => e.stopPropagation()} />
            <div className="img-viewer-label">{fullImg.label}</div>
          </div>
        )}
      </div>
    </>
