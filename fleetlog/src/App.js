...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .receipt-thumb { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; border: 1px solid var(--border); flex-shrink: 0; }
  ...
  .quick-btn { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; padding: 18px 14px; display: flex; flex-direction: column; gap: 8px; cursor: pointer; transition: all 0.2s; }
  .quick-btn:hover { border-color: var(--accent); }
  .quick-btn .qb-icon { font-size: 24px; }
  .quick-btn .qb-label { font-size: 14px; font-weight: 500; }
  ...
  .form-input { width: 100%; background: var(--surface2); border: 1px solid var(--border); border-radius: 12px; padding: 14px; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 16px; outline: none; transition: border 0.2s; }
  ...
  .btn-primary { width: 100%; background: var(--accent); color: var(--bg); border: none; border-radius: 14px; padding: 16px; font-family: 'DM Sans', sans-serif; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.2s; margin-top: 8px; }
  .btn-primary:hover { background: var(--accent2); }
  .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
  .btn-secondary { width: 100%; background: transparent; color: var(--muted); border: 1px solid var(--border); border-radius: 14px; padding: 15px; font-family: 'DM Sans', sans-serif; font-size: 15px; cursor: pointer; margin-top: 8px; }
  .receipt-upload { border: 2px dashed var(--border); border-radius: 14px; padding: 24px 16px; display: flex; flex-direction: column; align-items: center; gap: 8px; cursor: pointer; transition: all 0.2s; background: var(--surface2); position: relative; min-height: 110px; justify-content: center; }
  ...
  .receipt-upload .ru-icon { font-size: 28px; }
  .receipt-upload .ru-label { font-size: 13px; color: var(--muted); text-align: center; line-height: 1.4; }
  .receipt-upload .ru-sub { font-size: 11px; color: var(--muted); opacity: 0.6; }
  .receipt-preview { width: 100%; border-radius: 10px; max-height: 180px; object-fit: cover; display: block; }
  .receipt-remove { position: absolute; top: 8px; right: 8px; width: 30px; height: 30px; background: var(--danger); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; cursor: pointer; color: white; border: none; }
  .record-detail-img { width: 100%; border-radius: 14px; margin-bottom: 14px; max-height: 260px; object-fit: cover; cursor: pointer; }
  ...
  .transfer-btn { margin-top: 14px; background: var(--accent); color: var(--bg); border: none; border-radius: 12px; padding: 12px 22px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; cursor: pointer; }
  .photos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 16px; }
  .photo-slot { aspect-ratio: 1; border-radius: 12px; overflow: hidden; background: var(--surface2); border: 1px dashed var(--border); display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; gap: 4px; transition: border 0.2s; min-height: 90px; }
  .photo-slot:hover { border-color: var(--accent); }
  .photo-slot > span:first-child { font-size: 28px; }
  .photo-slot > span:last-child { font-size: 10px; color: var(--muted); text-align: center; padding: 0 4px; }
  .photo-slot img { width: 100%; height: 100%; object-fit: cover; }
  .manual-card { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; padding: 18px 16px; margin-bottom: 10px; display: flex; gap: 12px; align-items: center; cursor: pointer; transition: all 0.2s; }
  .manual-card:hover { border-color: var(--accent); }
  .manual-icon { width: 44px; height: 44px; background: var(--surface2); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; flex-shrink: 0; }
  .mc-title { font-size: 14px; font-weight: 500; margin-bottom: 2px; }
  .mc-sub { font-size: 12px; color: var(--muted); }
  .back-btn { display: flex; align-items: center; gap: 6px; color: var(--muted); font-size: 13px; cursor: pointer; margin-bottom: 16px; padding: 4px 0; }
  ...
  .chip { padding: 8px 14px; border-radius: 20px; font-size: 13px; font-weight: 500; border: 1px solid var(--border); background: var(--surface2); cursor: pointer; transition: all 0.15s; }
  ...
  .toast { position: fixed; bottom: 90px; left: 50%; transform: translateX(-50%); background: var(--accent); color: var(--bg); padding: 12px 22px; border-radius: 100px; font-size: 14px; font-weight: 600; z-index: 999; animation: fadeIn 0.2s; white-space: nowrap; }
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#0d0f0e", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
        ...
        <div className="config-warn">
          ...
          <p>Adicione no Netlify:<br/><br/><code>REACT_APP_SUPABASE_URL</code><br/><code>REACT_APP_SUPABASE_ANON_KEY</code></p>
        </div>
      </div>
    </>
  )
...
function FleetApp({ session }) {
  ...
  useEffect(() => {
    fetchVehicles(user.id)
      .then(async data => {
        ...
        setLoading(false)
        const all = {}
        await Promise.all((data||[]).map(v => fetchRecords(v.id).then(r => { all[v.id] = r||[] }).catch(()=>{})))
        setRecords(all)
      })
  ...
  const saveRecord = async () => {
    ...
    try {
      ...
      if (editRecord) {
        ...
        setRecords(r => ({...r,[activeVehicle.id]:(r[activeVehicle.id]||[]).map(rec=>rec.id===updated.id?updated:rec)}))
        setEditRecord(null); showToast("Registro atualizado ✓")
      } else {
    ...
    } catch(e) { console.error(e); showToast("Erro ao salvar registro") } finally { setSaving(false) }
  }
  ...
  const handlePhotoCapture = async (e, vehicleId, angle) => {
    ...
    try {
      ...
      const { data: fresh } = await supabase.from('vehicles').select('photos').eq('id',vehicleId).single()
      const newPhotos = {...(fresh?.photos||{}),[angle]:url}
      const updated = await updateVehicle(vehicleId, {photos:newPhotos})
      setVehicles(p => p.map(v => v.id===updated.id ? updated : v))
      showToast("Foto salva ✓")
    } catch(err) { console.error(err); showToast("Erro ao enviar foto") }
  }
  ...
  const HomeTab = () => (
    <div>
      ...
      {vehicles.map(v => {
        ...
        return (
          <div key={v.id} className={`car-card ${selectedId===v.id?"active-card":""}`} onClick={()=>{setSelectedId(v.id);setTab("detail")}}>
  ...
  // ── DETAIL ────────────────────────────────────────────────────────────────
  const DetailTab = () => {
    const [localFullImg, setLocalFullImg] = React.useState(null)

    if (!activeVehicle) return (
      <div style={{textAlign:"center",padding:"40px 0",color:"var(--muted)"}}>
        <div style={{fontSize:40,marginBottom:12}}>🚗</div><p>Selecione um veículo na aba Início</p>
      </div>
    )
    ...
    return (
      <div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          ...
          <span style={{fontSize:52}}>{ti.emoji}</span>
        </div>

        {activeVehicle.fipe_price && (
          <div className="fipe-value-card" style={{marginBottom:14}}>
            <div>
              <div style={{fontSize:10,textTransform:"uppercase",letterSpacing:1,color:"var(--muted)",marginBottom:2}}>💰 Valor FIPE</div>
              <div style={{fontFamily:"DM Serif Display, serif",fontSize:26,color:"var(--accent)"}}>{activeVehicle.fipe_price}</div>
              {activeVehicle.fipe_ref&&<div style={{fontSize:11,color:"var(--muted)",marginTop:2}}>Ref: {activeVehicle.fipe_ref}</div>}
            </div>
            ...
            return (
              <div className="timeline-item" key={rec.id} onClick={()=>setViewRecord(rec)}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                  ...
                  {rec.receipt_url&&(
                    <img src={rec.receipt_url} alt="NF" className="receipt-thumb" style={{cursor:"pointer"}}
                      onClick={e=>{e.stopPropagation();setLocalFullImg({url:rec.receipt_url,label:"Nota Fiscal — "+rec.title})}}/>
                  )}
                  <button onClick={e=>{e.stopPropagation();setEditRecord(rec);setRecordType(rec.type);setFormData({title:rec.title,date:rec.date,km:rec.km,cost:rec.cost,notes:rec.notes,parts:rec.parts});setReceiptPreview(rec.receipt_url||null);setReceiptFile(null);setModal("addRecord")}}
                    style={{background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"5px 12px",fontSize:12,color:"var(--muted)",cursor:"pointer"}}>
                    ✏️ editar
                  </button>
                </div>
              </div>
        ...
        {localFullImg&&(
          <div onClick={()=>setLocalFullImg(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.95)",zIndex:500,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
            <button onClick={()=>setLocalFullImg(null)} style={{position:"absolute",top:16,right:16,background:"rgba(255,255,255,0.15)",border:"none",borderRadius:"50%",width:44,height:44,color:"white",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
            <img src={localFullImg.url} alt={localFullImg.label} onClick={e=>e.stopPropagation()} style={{maxWidth:"100%",maxHeight:"80vh",objectFit:"contain",borderRadius:12}}/>
            <div style={{color:"rgba(255,255,255,0.6)",fontSize:13,marginTop:16}}>{localFullImg.label}</div>
          </div>
        )}
      </div>
    )
  ...
  const PhotosTab = () => {
    ...
    return (
      <div>
        ...
        <div style={{background:"rgba(200,245,58,0.06)",border:"1px solid rgba(200,245,58,0.15)",borderRadius:14,padding:"12px 14px",marginBottom:16,fontSize:12,color:"var(--muted)",lineHeight:1.5}}>
          📸 Fotos salvas na nuvem — aparecem para o novo dono na transferência.
        </div>
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {modal==="km"&&(
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div className="sheet">
              ...
              <div className="form-group"><label className="form-label">Novo KM</label><input className="form-input" type="number" placeholder="Digite o novo KM" value={newKm} onChange={e=>setNewKm(e.target.value)} style={{fontSize:20,textAlign:"center"}}/></div>
          ...
          <div className="overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
            <div className="sheet">
              ...
              <div style={{background:"rgba(200,245,58,0.06)",border:"1px solid rgba(200,245,58,0.15)",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
                <div style={{fontSize:12,color:"var(--muted)",lineHeight:1.8}}>✓ {activeRecords.length} registros<br/>✓ {activeRecords.filter(r=>r.receipt_url).length} notas fiscais<br/>✓ {Object.keys(activeVehicle?.photos||{}).length} fotos{activeVehicle?.fipe_price&&`\n✓ FIPE: ${activeVehicle.fipe_price}`}</div>
              </div>
            ...
            <div className="sheet">
              ...
              {viewRecord.receipt_url&&(
                <div style={{marginBottom:14}}>
                  <div className="form-label" style={{marginBottom:8}}>🧾 Nota Fiscal <span style={{fontSize:11,color:"var(--accent)",fontWeight:400}}>— toque para ampliar</span></div>
                  <img src={viewRecord.receipt_url} alt="NF" className="record-detail-img"
                    onClick={()=>setFullImg({url:viewRecord.receipt_url,label:"Nota Fiscal — "+viewRecord.title})}/>
                </div>
              )}
        ...
        {/* FULLSCREEN IMAGE (from viewRecord modal) */}
        {fullImg&&(
          <div onClick={()=>setFullImg(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.95)",zIndex:500,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
            <button onClick={()=>setFullImg(null)} style={{position:"absolute",top:16,right:16,background:"rgba(255,255,255,0.15)",border:"none",borderRadius:"50%",width:44,height:44,color:"white",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
            <img src={fullImg.url} alt={fullImg.label} onClick={e=>e.stopPropagation()} style={{maxWidth:"100%",maxHeight:"80vh",objectFit:"contain",borderRadius:12}}/>
            <div style={{color:"rgba(255,255,255,0.6)",fontSize:13,marginTop:16}}>{fullImg.label}</div>
          </div>
        )}
